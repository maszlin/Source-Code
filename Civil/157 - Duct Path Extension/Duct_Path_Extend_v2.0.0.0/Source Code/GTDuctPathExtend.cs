...
{
    class GTDuctPathExtend : Intergraph.GTechnology.Interfaces.IGTCustomCommandModeless
    {
        ...
        GTWindowsForm_DuctPathExtend m_CustomForm = null;
        ...
        void m_oIGTCustomCommandHelper_MouseMove(object sender, GTMouseEventArgs e)
        {
            try{
               ...
               // startdraw = 100;
                #region Get Selected Stub/Stump Duct Path
                if (startdraw == 100)
                {
                    m_gtapp.SetStatusBarText(GTStatusPanelConstants.gtaspcMessage, "Select Stub/Stump Duct Path to extend. Right Click to exit application!");
                    ...
                    {
                        MessageBox.Show("Selected more than one features!\nSelect Stub/Stump Duct Path to extend!", "Duct Path Extension", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        m_gtapp.SelectedObjects.Clear();
                        return;
                    }
                    return;
                ...
                if (startdraw == 39)
                {
                    ...
                    {
                        //MessageBox.Show("Please, place Label on creating Conduit Feature!", "Duct Path Extension", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                    else
                    {
                        ...
                        if (m_CustomForm.SectSlashes.Count > 0)
                        {
                            if (m_CustomForm.LastSection != 1)
                                ...
                                {
                                   // MessageBox.Show("Not allowed to place Label for new section on existing one!", "Duct Path Extension", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    return;
                                }
            ...
            try
            {
                ...
                if (e.Button != 2)
                {
                    ...
                    if (startdraw == 20)
                    {
                        ...
                        if (projectPoint.X == 0 && projectPoint.Y == 0)
                        {
                            MessageBox.Show("Please, place Slash Point on creating Conduit Feature!", "Duct Path Extension", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            return;
                        }
                        else
                        {
                            ...
                            if (m_CustomForm.SectSlashes.Count > 0)
                            {
                                if (length <= m_CustomForm.SectSlashes[m_CustomForm.SectSlashes.Count - 1].length)
                                {
                                    MessageBox.Show("Not allowed place Slash for new section on existing one!", "Duct Path Extension", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    return;
                                }
                            }
                    ...
                    if (startdraw == 2 || startdraw == 20)
                    {
                        ...
                        else
                        {
                            ...
                            if (m_CustomForm.SectSlashes.Count > 0)
                            {
                                if (length <= m_CustomForm.SectSlashes[m_CustomForm.SectSlashes.Count - 1].length)
                                {
                                    MessageBox.Show("Not allowed to place Slash for new section on existing one!", "Duct Path Extension", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    return;
                                }
                            }
        ...
        public void Activate(Intergraph.GTechnology.API.IGTCustomCommandHelper CustomCommandHelper)
        {
            ...
            m_gtapp.SetStatusBarText(GTStatusPanelConstants.gtaspcMessage, "Running Duct Path Extension ...");            
            ...
            SubscribeEvents();
             ...
             m_CustomForm = new GTWindowsForm_DuctPathExtend();
        ...
        public bool CanTerminate
        {
            get
            {
                if (startdraw == 0)
                    m_CustomForm.Hide();
                DialogResult retVal = MessageBox.Show("Do you want to discard your current changes and exit?", "Duct Path Extension", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (retVal == DialogResult.Yes)
                {
                    ExitCmd();
                    //  return true;
