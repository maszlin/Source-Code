...
namespace AG.GTechnology.Utilities
{
    public class PGeoLib
    {
        ...
        public static double GetOrientedPointAngle(IGTGeometry geom)
        {
            double TempAngle = 0;
            IGTOrientedPointGeometry pointGeom = (IGTOrientedPointGeometry)geom;
            double X1 = 0;
            double Y1 = 0;
            double X2 = pointGeom.Orientation.I * pointGeom.Orientation.Magnitude;
            double Y2 = pointGeom.Orientation.J * pointGeom.Orientation.Magnitude;

            TempAngle = GetAngle(X1, Y1, X2, Y2);
        ...
        public static double GetAngle(IGTPoint point1, IGTPoint point2)
        {
            double _Angle;

            _Angle = GetAngle(point1.X, point1.Y, point2.X, point2.Y);
        ...
        public static double GetLength(IGTPoint point1, IGTPoint point2)
        {
            double lenght;

            lenght = GetLength(point1.X, point1.Y, point2.X, point2.Y);
        ...
        public static double GetLength(IGTGeometry pline)
        {
            double lenght = 0;

            for (int i = 0; i < pline.KeypointCount - 1; i++)
            {
                lenght = lenght + GetLength(pline.GetKeypointPosition(i).X, pline.GetKeypointPosition(i).Y, pline.GetKeypointPosition(i + 1).X, pline.GetKeypointPosition(i + 1).Y);
            }
        ...
        public static IGTPoint GetPoint(IGTGeometry pline, double dist)
        {
            double lenght = 0;
            IGTPoint pnt = GTClassFactory.Create<IGTPoint>();
            ...
            for (int i = 0; i < pline.KeypointCount - 1; i++)
            {
                double dLine = GetLength(pline.GetKeypointPosition(i).X, pline.GetKeypointPosition(i).Y, pline.GetKeypointPosition(i + 1).X, pline.GetKeypointPosition(i + 1).Y);
                if ((lenght + dLine) == dist) return pline.GetKeypointPosition(i + 1);
                if ((lenght + dLine) > dist)
                {
                    pnt.X = pline.GetKeypointPosition(i).X + ((dist - lenght) / dLine) * (pline.GetKeypointPosition(i + 1).X - pline.GetKeypointPosition(i).X);
                    pnt.Y = pline.GetKeypointPosition(i).Y + ((dist - lenght) / dLine) * (pline.GetKeypointPosition(i + 1).Y - pline.GetKeypointPosition(i).Y);
                    break;
                }
        ...
        public static IGTPoint GetProjectedPoint(IGTGeometry pline, IGTPoint pnt)
        {
            List<IGTPoint> _list = new List<IGTPoint>();
            double minLen = 0;
            IGTPoint prjPoint = null;
            ...
            {
                IGTSegment segment = GTClassFactory.Create<IGTSegment>();
                segment.Point1 = pline.GetKeypointPosition(i);
                segment.Point2 = pline.GetKeypointPosition(i + 1);
        ...
        public static double GetProjectedAngle(IGTGeometry pline, IGTPoint pnt)
        {
            double minLen = 0;
            double prjAngle = 0;
            minLen = GetLength(pline.GetKeypointPosition(0).X, pline.GetKeypointPosition(0).Y, pnt.X, pnt.Y);
            prjAngle = GetAngle(pline.GetKeypointPosition(0).X, pline.GetKeypointPosition(0).Y,
                pline.GetKeypointPosition(1).X, pline.GetKeypointPosition(1).Y);
            for (int i = 0; i < pline.KeypointCount - 1; i++)
            {
                IGTSegment segment = GTClassFactory.Create<IGTSegment>();
                segment.Point1 = pline.GetKeypointPosition(i);
                ...
                if (minLen > GetLength(pnt2.Point.X, pnt2.Point.Y, pnt.X, pnt.Y))
                {
                    minLen = GetLength(pnt2.Point.X, pnt2.Point.Y, pnt.X, pnt.Y);
                    prjAngle = GetAngle(segment.Point1.X, segment.Point1.Y, segment.Point2.X, segment.Point2.Y);
                }
            }
            return prjAngle;
        }
        ...
        public static IGTPolylineGeometry CreateHeaderLine(IGTPoint ArrowPoint, IGTPoint TextPoint)
        {
            IGTPolylineGeometry oLine = GTClassFactory.Create<IGTPolylineGeometry>();

            oLine.Points.Add(ArrowPoint);
            IGTPoint pnt = GTClassFactory.Create<IGTPoint>();

            if (TextPoint.X > ArrowPoint.X)
            {
                pnt.X = TextPoint.X - 1;
                pnt.Y = TextPoint.Y;
            }
        ...
        public static void DeleteComponents(short iFNO, int iFID)
        {
            string sFeatureName = "";
            ...
            oFeatureRS.Filter = "G3E_FNO = " + iFNO;
        ...
        public static void MoveComponent(IGTKeyObject fromFeature, short iCNO, IGTVector vector)
        {
            while (!fromFeature.Components.GetComponent(iCNO).Recordset.EOF)
            {
                IGTGeometry geom = fromFeature.Components.GetComponent(iCNO).Geometry;
                geom.Move(vector);

                fromFeature.Components.GetComponent(iCNO).Geometry = geom;
                fromFeature.Components.GetComponent(iCNO).Recordset.MoveNext();
            }
        }

        public static void MoveFeature(IGTKeyObject fromFeature, IGTVector vector)
        {
            for (int i = 0; i < fromFeature.Components.Count; i++)
            {
                if (!fromFeature.Components[i].Recordset.EOF)
                {
                    if (fromFeature.Components[i].Geometry != null)
                    {
                        while (!fromFeature.Components[i].Recordset.EOF)
                        {
                            IGTGeometry geom = fromFeature.Components[i].Geometry;
                            geom.Move(vector);

                            fromFeature.Components[i].Geometry = geom;
                            fromFeature.Components[i].Recordset.MoveNext();
                        }
                    }
                }
            }
