...
class myUtil
{
    ...
    public static ADODB.Recordset FindFID(string tablename, int FID)
    {
        string ssql = "SELECT * FROM " + tablename + " WHERE G3E_FID = " + FID.ToString();
        ADODB.Recordset rs = ADODB_ExecuteQuery(ssql);
        return (rs);
    }
    public static ADODB.Recordset FindRecord(string tablename, string filter)
    {
        string ssql = "SELECT * FROM " + tablename + " WHERE " + filter;
        ADODB.Recordset rs = ADODB_ExecuteQuery(ssql);
        return (rs);
    }
    public static ADODB.Recordset ADODB_ExecuteQuery(string ssql)
    {
        return GTClassFactory.Create<IGTApplication>().DataContext.OpenRecordset(ssql,
