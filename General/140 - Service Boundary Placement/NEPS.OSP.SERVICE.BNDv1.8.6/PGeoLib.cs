...
namespace NEPS.OSP.SERVICE.BND
{
    public class PGeoLib
    {
        ...
        /// <summary>
        /// CreatePolyGeom - create and return polygon geometry
        /// 
        /// created : m.zam 14-mar-2012
        /// </summary>
        /// <param name="PointCol">points for the polygon</param>
        /// <returns>polygon geometry</returns>
        public static IGTPolygonGeometry CreatePolyGeom(List<IGTPoint> PointCol)
        {
            IGTPolygonGeometry oBoundaryLine;
            oBoundaryLine = GTClassFactory.Create<IGTPolygonGeometry>();
            for (int i = 0; i < PointCol.Count; i++)
                oBoundaryLine.Points.Add(PointCol[i]);

            //oNewFeature.Components.GetComponent(iCNO).Geometry = oBoundaryLine;
            return oBoundaryLine;
        }
        ...
        public static IGTPolygonGeometry CreatePolygonGeom(List<IGTPoint> PointCol)
        {
            IGTPolygonGeometry oBoundaryLine;
            oBoundaryLine = GTClassFactory.Create<IGTPolygonGeometry>();
            for (int i = 0; i < PointCol.Count; i++)
