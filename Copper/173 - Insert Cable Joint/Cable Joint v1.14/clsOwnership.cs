...
class clsOwnership
{
    ...
    public void ReadChilds(int parentFID)
    {
        ...
        if (parentID > -1)
        {
            ...
            while (!rs.EOF)
            {
                ...
                //switch (childFNO)
                //{
                //    case SHELF_FNO: ReadFrame(childFID); break;
                //    case SLOT_FNO: ReadSlot(childFID); break;
                //    case CARD_FNO: ReadCard(childFID); break;
                //}
                rs.MoveNext();
            }
            rs.Close();
    ...
    public void DeleteRecord(int recFID, int recFNO)
    {
        try
        {
            ...
            //{
            //    case SHELF_FNO:
            //        ssql = "DELETE FROM GC_SHELF WHERE G3E_FID = " + recFID.ToString(); break;
            //    case SLOT_FNO:
            //        ssql = "DELETE FROM GC_SLOT WHERE G3E_FID = " + recFID.ToString(); break;
            //    case CARD_FNO:
            //        ssql = "DELETE FROM GC_CARD WHERE G3E_FID = " + recFID.ToString(); break;
            //    default:
            //        return;
            //}
