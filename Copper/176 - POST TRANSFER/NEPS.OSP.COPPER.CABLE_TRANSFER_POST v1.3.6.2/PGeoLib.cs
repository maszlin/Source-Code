...
public class PGeoLib
{
    ...
    public static void DeleteComponents(short iFNO, int iFID)
    {
        ...
        ADODB.Recordset oFeatureRS = 
            GTClassFactory.Create<IGTApplication>().DataContext.MetadataRecordset("G3E_FEATURECOMPS_OPTABLE");
        ...
        if (!(oFeatureRS.BOF && oFeatureRS.EOF))
        {
            ...
            while (!oFeatureRS.EOF)
            {
                ...
                if (sFeatureName.Contains("GC_CBL"))
                    System.Diagnostics.Debug.WriteLine("Table Name : " + sFeatureName);
                else
                    System.Diagnostics.Debug.WriteLine("Table Name : " + sFeatureName);
                string ssql = "DELETE FROM " + sFeatureName + " WHERE G3E_FNO=" + iFNO + " AND G3E_FID = " + iFID;
                GTClassFactory.Create<IGTApplication>().DataContext.Execute(ssql, out recordsAffected, 1, null);
