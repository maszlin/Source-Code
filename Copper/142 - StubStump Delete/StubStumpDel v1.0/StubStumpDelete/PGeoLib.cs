        ...
        public static void DeleteComponents(IGTKeyObject fromFeature)
        {
            for (int i = 0; i < fromFeature.Components.Count; i++)
            {
                ...
                if (!fromFeature.Components[i].Recordset.EOF)
                {
                    ...
                    application.DataContext.Execute("DELETE FROM " + fromFeature.Components[i].TableName + " WHERE G3E_FNO=" + fromFeature.FNO.ToString() + " AND G3E_FID=" + fromFeature.FID.ToString(), out recordsAffected, 1, null);
                }
            }
        }
