...
namespace NEPS.OSP.COPPER.COLLECT.COUNT
{
    class clsMoveLabel
    {
        ...
        private void SaveLabel()
        {
            try
            {
                GTCollectCount.m_oIGTTransactionManager.Begin("Count");
                GTCollectCount.m_gtapp.BeginWaitCursor();
            ...
            catch (Exception ex)
            {
                ...
                GTCollectCount.m_CustomForm.TopMost = false;
                //   MessageBox.Show("Fail saving record to NEPS\r\n" + ex.Message);
            }
            finally
            {
                GTCollectCount.m_oIGTTransactionManager.RefreshDatabaseChanges();
                GTCollectCount.m_gtapp.EndWaitCursor();
                GTCollectCount.m_CustomForm.TopMost = true;
            }
