...
{
    public class GetYearInstall : Intergraph.GTechnology.Interfaces.IGTFunctional
    {
        ...
        public void Execute()
        {
            try
            {
                ...
                strSql = "SELECT YEAR_INSTALL FROM G3E_JOB WHERE G3E_IDENTIFIER = '" + strJobID + "'";
                ...
                {
                    rsSQL.MoveFirst();
                    oComp.Recordset.Update("YEAR_PLACED", rsSQL.Fields[0].Value.ToString());
                }
                else
                {
                    rsSQL.MoveFirst();
                    oComp.Recordset.Update("YEAR_PLACED", "");
                }
                rsSQL = null;
                strSql = null;
            }
