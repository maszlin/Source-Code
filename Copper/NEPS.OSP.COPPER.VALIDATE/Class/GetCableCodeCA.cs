...
{
    public class GetCableCodeCA : Intergraph.GTechnology.Interfaces.IGTFunctional
    {
        ...
        public void Execute()
        {
            try
            {
                ...
                /*
                strSql = "SELECT SCHEME_NAME FROM G3E_JOB WHERE G3E_IDENTIFIER = '" + strJobID + "'";
                ...
                {
                    rsSQL.MoveFirst();
                    oComp.Recordset.Update("SCHEME_NAME", rsSQL.Fields[0].Value.ToString());
                }
                else
                {
                    rsSQL.MoveFirst();
                    oComp.Recordset.Update("SCHEME_NAME", "");
                }
                rsSQL = null;
                strSql = null;
                */
