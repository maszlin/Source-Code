...
{
    public class GetExcAbb : Intergraph.GTechnology.Interfaces.IGTFunctional
    {
        ...
        public void Execute()
        {
            try
            {
                ...
                strSql = "SELECT EXC_ABB FROM G3E_JOB WHERE G3E_IDENTIFIER = '" + strJobID + "'";
                ...
                {
                    rsSQL.MoveFirst();
                    oComp.Recordset.Update("EXC_ABB", rsSQL.Fields[0].Value.ToString());
                }
                else
                {
                    rsSQL.MoveFirst();
                    oComp.Recordset.Update("EXC_ABB", "");
                }
                rsSQL = null;
                strSql = null;
            }
