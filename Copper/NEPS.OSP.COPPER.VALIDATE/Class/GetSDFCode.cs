...
{
    public class GetSDFCode : Intergraph.GTechnology.Interfaces.IGTFunctional
    {
        ...
        public void Execute()
        {
            try
            {
                ...
                strSql = "SELECT GC_OSP_COP_VAL.CNO_10301_GET_ITFACE_CODE('" + strJobID + "', 'SDF') FROM DUAL";
                ...
                {
                    rsSQL.MoveFirst();
                    oComp.Recordset.Update("SDF_CODE", rsSQL.Fields[0].Value.ToString());
                }
                else
                {
                    rsSQL.MoveFirst();
