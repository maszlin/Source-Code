...
{
    public class GetCableCode : Intergraph.GTechnology.Interfaces.IGTFunctional
    {
        ...
        #region GetCable Code

        public static string DCableCode(IGTKeyObject objFeat)
        {
            try
            {
                IGTComponent oComp = objFeat.Components.GetComponent(53);

                oComp.Recordset.MoveFirst();
                int cblFID = int.Parse(oComp.Recordset.Fields["G3E_FID"].Value.ToString());
                int InFID = int.Parse(oComp.Recordset.Fields["IN_FID"].Value.ToString());

                string ssql = "SELECT GC_OSP_COP_VAL.CNO_7000_GET_DSIDE_CBLCODE(" + cblFID + "," + InFID + ") AS DCODE FROM DUAL";
                ...
                if (rsSQL.RecordCount > 0)
                {
                    ...
                    oComp = objFeat.Components.GetComponent(53);
